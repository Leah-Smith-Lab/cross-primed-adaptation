...
EOP<-
  ...
  stat_summary(fun = mean, geom = "col", aes(fill = Spacer), color = "black") + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.4, linewidth=0.3) +
geom_jitter(color = "black", shape = 1, alpha = 0.6, width=0.1)+
...
JS26_escape_titers_growth_curves_plot <-
  ...
  stat_summary(fun = mean, geom = "col", aes(fill=Plasmid_spacer), color = "black") + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.4, linewidth=0.3) +
geom_jitter(color = "black", shape = 1, alpha = 0.6, width=0.1)+
...
sheet="2025_06_17_growth_curves")%>%
  pivot_longer(
    cols = `1`:`145`,       # columns to gather (measurment cycles; every 10 min for 24 h)
    names_to = "Cycle",     # new column for cycles
    values_to = "OD600"     # new column for values (absorbance at 600nm)
  ) %>%
  mutate(
    Cycle = as.numeric(Cycle),
    Time = ((Cycle - 1) * 10) / 60,  # Convert measurments to hours
...
```
## Filter data so controls can be overlayed on plots during faceting
```{r}
...
```
## I-E growth curve plots - adjust MOI field to plot MOI== 1 or 0.1
```{r}
...
```
## Plot JS26 and escape phage MOIs on LacA (WT strain)
```{r}
