...
# Create empty list that processed data will be written to after processing
normalized_dfs <- list()
...
for (i in seq_along(joined_dfs)) {
  ...
  # Directly process and store the result in the list, no intermediate variable
...
for (df_name in names(normalized_dfs)) {
  ...
  df_binned <- df %>%
    ...
    summarise(
      ...
      sum_combined = sum(coverage_ratio_combined, na.rm = TRUE)  # Sum of coverage_ratio_minus for each bin
    ) %>%
    mutate(
...
for (df_name in names(normalized_relative_position_dfs)) {
  ...
  priming <- str_extract(df_name, "(?<=_)([^_]+_[^_]+)(?=_C)")  # This captures the part before '_C' (e.g., 'I_F' from 'I_F_C3')
...
for (df_name in names(collapsed_absolute_position_dfs)) {
  ...
  # Reshape the data into long format for stacking within the same loop
