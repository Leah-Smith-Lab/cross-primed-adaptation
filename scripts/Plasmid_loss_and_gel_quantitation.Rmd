...
# read in plasmid loss data (% mCherry +/-)
...
# plotting all data combined by priming condition #
...
type_III_overexpression_CRISPR1_array <- read_xlsx("./input/priming_gel_quantitations/Adaptation_gel_quantitation.xlsx", sheet = "2024-04-05_CRISPR1_expansion") %>%
  ...
  # calculate the average Band % for each 'band_number', 'expression', and 'priming'
  ...
  # calculate the control average Band % for each 'band_number'
  group_by(band_number) %>%
  mutate(
    control_avg = mean(band_percent[expression == "control"], na.rm = TRUE)  # Control average per band_number
  ) %>%
  # normalize Band % for 'I-E' and 'I-F' relative to the control average
  mutate(
...
# summarize the data to get one row per band_number, expression, priming, and array
...
type_III_overexpression_CRISPR2_array <- read_xlsx("./input/priming_gel_quantitations/Adaptation_gel_quantitation.xlsx", sheet = "2024-04-05_CRISPR2_expansion") %>%
  ...
  mutate(
    ...
    array = 2         # Add new column 'array' and set it to 2 for all rows
  ) %>%
  # calculate the average Band % for each 'band_number', 'expression', and 'priming'
  group_by(band_number, expression, priming, array) %>%  # Include 'array' in the grouping
  mutate(
...
```{r}
# type I-E and I-F overexpression in CRISPR1 (type I-E priming)
  # read the Excel file and preprocess the data
...
# type I-E and I-F overexpression in CRISPR2  (type I-F priming)
  # read the Excel file and preprocess the data
